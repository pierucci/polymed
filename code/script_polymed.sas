...
CASE 
       WHEN &var IN (&m1, &m2, &m3) THEN '1'
       WHEN &var IN (&m4, &m5, &m6) THEN '2'
       WHEN &var IN (&m7, &m8, &m9) THEN '3'
       WHEN &var IN (&m10, &m11, &m12) THEN '4'
       ELSE &var
END 
%MEND ;
...
/* comme son nom l'indique cette macro genere un referentiel pharma mis a jour
en allant chercher automatiquement l'info dans ORAVUE ou SPDREF
selon le profil de connection

quand les 2 librairies sont disponibles, ORAVUE est selectionne en priorite
*/

%MACRO generate_ref_pha() ;

%IF %SYSFUNC(exist(oravue.ir_pha_r, data))
%THEN %LET ref_orig = oravue.ir_pha_r ;

%ELSE %IF %SYSFUNC(exist(spdref.ir_pha_r, data)) 
%THEN %LET ref_orig = spdref.ir_pha_r ;

%ELSE %PUT ERROR: tables ORAVUE.ir_pha_r et SPDREF.ir_pha_r introuvables. ;
...
PROC SQL ;
	...
	AND (pha_prs_ide NOT BETWEEN 0 AND 1999999)
...
pour la population d'int�r�t
avec une ligne par individu, classe atc par jour

et �galement extraction du nombre de trimestre
o� il existe au moins une prestation pharmacie ou ucd
par individu
*/

%LET PHA_RPHA =
(SELECT REF_PHA.atc, REF_PHA.nbr_moi, ES_PHA_F.cle_tec_jnt
FROM        spdebs.es_pha_f         ES_PHA_F
INNER JOIN  spduser.&prefix.ref_pha REF_PHA
    ON ES_PHA_F.pha_prs_ide = REF_PHA.pha_prs_ide)
;

%LET UCD_RPHA =
(SELECT REF_PHA.atc, REF_PHA.nbr_moi, ES_UCD_F.cle_tec_jnt
FROM        spdebs.es_ucd_f         ES_UCD_F
INNER JOIN  spduser.&prefix.ref_pha REF_PHA
    ON ES_UCD_F.ucd_ucd_cod = REF_PHA.cod_ucd_chr
WHERE ES_UCD_F.ucd_top_ucd = 0)
;

%LET PRS_PHA =
(SELECT * FROM (&PHA_RPHA) UNION (&UCD_RPHA))
;

PROC SQL INOBS = 0 _method  ;
    ...
    SELECT  ANO.ben_nir_idt,
            ...
            %trimestre(&annee, ES_PRS_F.exe_soi_amd) AS trm,
            MAX(PRS_PHA.nbr_moi) AS nbr_moi

    FROM        spduser.&nir        ANO
    INNER JOIN  spdebs.es_prs_f     ES_PRS_F
        ON  %join_nir(ES_PRS_F, ANO)

    LEFT JOIN (&PRS_PHA)            PRS_PHA
        ON  %join_egb(ES_PRS_F, PRS_PHA)
...
PROC SQL INOBS = MAX _method ;
    ...
    SELECT ANO.ben_nir_idt,
           ...
           COUNT(DISTINCT %trimestre(&annee, ES_PRS_F.exe_soi_amd)) AS n_trm /* combien de trimestres o� il y a des prestations, utilis� pour l'inclusion */
    ...
    WHERE   ES_PRS_F.exe_soi_dtd BETWEEN &dtd_exe_deb AND &dtd_exe_fin
            /* restriction de date de d�livrance sur l'ann�e */

            AND ES_PRS_F.prs_nat_ref IN &prs_pha
            /* s�lection uniquement des prestations pharma ou ucd */

            AND ES_PRS_F.dpn_qlf <> 71
            AND ES_PRS_F.prs_pai_mnt > 0
            /* exclusion des r�gularisations et des lignes pour info */
...
les deux requ�tes sont combin�es en une par l'expression GROUPING SETS
top_inc est = 1 pour la requ�te concernant les crit�res d'inclusion
*/
...
PROC SQL ;
    ...
    EXECUTE (
        ...
        SELECT  ANO.ben_nir_psa, ANO.ben_rng_gem,
                ...
                GROUPING(atc) AS top_inc, /* = 1 pour requ�te inclusion, = 0 pour requ�te m�dicaments */
                
                REF_PHA.atc,

                ER_PRS_F.exe_soi_dtd,
                %trimestre(&annee) AS trm, /* le trimestre d'�x�cution de la prestation */

                MAX(REF_PHA.nbr_moi) AS nbr_moi, /* si 2 m�dicaments de la m�me classe ATC sont d�livr�s le m�me jour,
                                                    on prend le plus grand conditionnement */

                COUNT(DISTINCT %trimestre(&annee)) AS n_trm /* combien de trimestres o� il y a des prestations, utilis� pour l'inclusion */
        ...
        LEFT JOIN   ref_pha         REF_PHA
            ...
            OR  ER_UCD_F.ucd_ucd_cod = REF_PHA.cod_ucd_chr /* on fait la jointure du r�f�rentiel sur le code CIP ou UCD, selon le mode de d�livrance */
        ...
        WHERE   ER_PRS_F.flx_dis_dtd BETWEEN TO_DATE(&dtd_flx_deb, 'DDMMYYYY') AND TO_DATE(&dtd_flx_fin, 'DDMMYYYY')
                ...
                /* exclusion des m�dicaments en sus du GHS */
